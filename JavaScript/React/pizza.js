...
// Webpack при сборке React проекта просто все файлы в один запихнёт для оптимизации. Для этого надо прописать npm run build.
// bundle это как раз этот один большой сжатый js файл. Chunk - это кусок кода из бандла.
// Code splitting надо использовать только тогда, когда хочешь чтобы бандл не содержал всю страницу, а грузил её отдельным файлом.
// Вообщем, code splitting это разделение большого жирного куска кода на более мелкие.

/* Если есть код, который не особо важный и он может загрузиться уже после загрузки основного кода, то можно сделать из него чанк.
Таким образом, основной код загрузиться быстрее и юзер быстрее увидит приложение.
Для этого используется динамический импорт и пишеться он примерно так: */
import('./math').then(math => { // Импортируем наш модуль и он вернёт промис, который и будет нашим модулем.
    console.log(math.add(1, 2)); // Обращаемся к функции add у нашего модуля.
})
// И теперь, эта часть будет грузиться отдельным файлом, а не вместе со всем бандлом.

// Все остальные страницы, кроме основной также лучше грузить чанками. Загрузка чанков называется lazy loading.
// Пример подгрузки компонента с помощью lazy loading:
const Cart = React.lazy(() => import('./component')); // Этот компонент загрузиться только тогда, когда будет отрендерен.

// Компоненты, использующие ленивую загрузку нужно объявлять следующим образом:
<Route path='/cart' element={
    // Этот компонент должен быть обёрнут в компонент React.Suspense и в fallback ему будет передано то, что будет отображаться вместо корзины, во время её загрузки.
    // Также, React.Suspence можно навесить на весь Routes, а не на каждый Route.
    <React.Suspense fallback={<div>Идёт загрузка корзины..</div>}>
        <Cart />
    </React.Suspense>
} />

// Чтобы у чанка было какстомное имя, нужно сделать так:
const Cart2 = React.lazy(() => import(/* webpackChunkName: "Cart" */ './component'));

// React.lazy() работает только с файлами, которые экспортируются по умолчанию.
// Но если файл использует обычный экспорт, то нужно сделать так:
const Cart4 = React.lazy(() => import('./component').then((m) => ({
    default: m.FullPizza
})));


/* Серверный рендеринг (Server-Side Rendering, SSR) — это техника, при которой веб-приложения отображаются на сервере, а затем отправляются на клиент.

В ReactJS серверный рендеринг позволяет выполнить код на сервере и отобразить результат в виде HTML-разметки. 
Это достигается с помощью функции `renderToString()` из пакета `react-dom/server`.

Преимущества серверного рендеринга:

Улучшение производительности: клиент получает уже готовую HTML-разметку и не тратит время на выполнение JavaScript-кода.
Улучшение SEO: поисковые системы могут лучше индексировать серверно отрендеренный контент, что способствует повышению рейтинга сайта в поисковых результатах.
Улучшение социального взаимодействия: серверный рендеринг обеспечивает корректное представление приложения для социальных сетей и сервисов предпросмотра ссылок. */

// Так вот, React.lazy и React.Suspense пока что не умеют работать с серверным рендерингом. Поэтому, для этого нужно использовать либу React Loadable.
// Пример:
import Loadable from 'react-loadable';
const Cart3 = Loadable({
    loader: () => import('./component'), // Что рендерить с помощью lazy loading
    loading: () => <div>Идёт загрузка корзины..</div> // Что показывать во время загрузки
})

// Tree Shaking - это когда у тебя есть какой то неиспользуемый код (типа импортов, которые нигде не юзаются), то такой код автоматически удаляется при билдинге.

// Чтобы не делать много импортов, можно создать конкретную точку index.ts, из которого будут экспортироваться все нужные нам файлы:
export { default as Skeleton } from 'Skeleton'; // Примерно так. Это называется Reexport.

// А для именнованных экспортов можно сделать так:
export * from 'Skeleton';

// Но лучше уж делать импорты для каждого по отдельности, чем так. Это можно использовать, но редко.